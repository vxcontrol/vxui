...
local function print_config()
    ...
    for attr, val in pairs(__config) do
        print("\t", attr, type(val))
        if type(val) == "userdata" and attr ~= "set_current_config" then
...
local function push_events(id)
    ...
    for event_name, _ in pairs(event_config) do
        __api.push_event(id, cjson.encode({
            name = event_name,
            data = generate_json(__config.get_event_data_schema()),
            uniq = event_name .. "_" .. random_data(5),
        }))
    end
end
